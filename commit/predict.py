...
def state_predict(states:torch.Tensor, actions:torch.Tensor, timesteps:torch.Tensor, masks:torch.Tensor, device, state_columns, action_columns) -> torch.Tensor:
    ...
    for index, key in enumerate(state_columns):
        ...
        if type == 'pos':
            states[:, :, index] = (value - pos_min_max_dic[joint][0]) / (
                    pos_min_max_dic[joint][1] - pos_min_max_dic[joint][0])
        elif type == 'vel':
            states[:, :, index] = (value - vel_min_max_dic[joint][0]) / (
                    vel_min_max_dic[joint][1] - vel_min_max_dic[joint][0])
    for index, key in enumerate(action_columns):
        value = actions[:, :, index]
        _, joint = key.split('_')
        actions[:, :, index] = ((value - pwm_min_max[joint]["positive"][0]) / (
                pwm_min_max[joint]["positive"][1] - pwm_min_max[joint]["positive"][0]) * (value > 0)
                                + (value - pwm_min_max[joint]["negative"][0]) / (
                                        pwm_min_max[joint]["negative"][1] - pwm_min_max[joint]["negative"][0]) * (
                                        value < 0))
    ...
    for index, key in enumerate(state_columns):
        ...
        type, joint = key.split('_')
        if type == 'pos':
            next_state[:, :, index] = value * (pos_min_max_dic[joint][1] - pos_min_max_dic[joint][0]) + \
                                      pos_min_max_dic[joint][0]
        elif type == 'vel':
            next_state[:, :, index] = value * (vel_min_max_dic[joint][1] - vel_min_max_dic[joint][0]) + \
                                      vel_min_max_dic[joint][0]
